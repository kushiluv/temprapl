...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        if (data.is_authenticated) {
          ...
          const detailsRes = await fetch(`/Farmer/${data.user_id}`);
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get('/user/orders'); // Adjust endpoint as needed
    ...
    const fetchListedProducts = async () => {
      try {
        const response = await axios.get('/user/listed-products'); // Adjust endpoint as needed
